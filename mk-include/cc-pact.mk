...
PACT_FILES_DIR ?= $(CURDIR)/test/pact/pacts
PACT_TESTS_ENABLED ?= true
PACT_REPORT_SUMMARY_FILE ?= $(BUILD_DIR)/pact-summary.txt
PACT_CONSUMER_TEST_REPORT_FILE ?= $(TEST_REPORT_FILE_PREFIX)-PACT-CONSUMER-TEST-$(TEST_REPORT_FILE_SUFFIX)
PACT_PROVIDER_TEST_REPORT_FILE ?= $(TEST_REPORT_FILE_PREFIX)-PACT-PROVIDER-TEST-$(TEST_REPORT_FILE_SUFFIX)
...
# Because if your consumer test failed, this is most likely a programming error local to your codebase.
	...
	set -o pipefail && \
	PACT_FILES_DIR=$(PACT_FILES_DIR) $(GO) test -v -count=1 -tags=pact.consumer $(PACT_TEST_DIR) -json | \
	PYRAMID_TEST_CATEGORY="pact-consumer" \
...
# See https://confluentinc.atlassian.net/wiki/spaces/ENV/pages/3157558462/Pact+in+CI
	...
	PYRAMID_TEST_CATEGORY="pact-provider" \
