...
# Controls individual task timeout in minutes for halyard deploy.sh
WAYFARER_TASK_TIMEOUT_IN_MINS ?= 

# Controls error budgets for halyard deploy.sh
# Note, it only would be used by halyard-auto-deploy-service target
WAYFARER_ERROR_BUDGET_COUNT ?= 
...
else ifeq ($(maxInflightCount),)
	$(eval max_inflight_count_flag := )
else
	$(eval max_inflight_count_flag := -max-inflight-count $(maxInflightCount))
endif

ifeq ($(taskTimeout),null)
	$(eval task_timeout_flag := )
else ifeq ($(taskTimeout),)
	$(eval task_timeout_flag := )
else
	$(eval task_timeout_flag := -task-timeout $(taskTimeout))
endif
...
# add to HALYARD_PRE_DEPLOY or HALYARD_POST_DEPLOY are included before or after halyard.mk
	...
	@echo "Deploying"
	$(deploy_sh) -service $(svc) -env $(env) -cluster-env $(CLUSTER_ENV) -version $(ver) $(sleep_flag) $(enable_wayfarer_flag) $(rollout_flag) $(percent_flag) $(error_budget_flag) $(max_inflight_count_flag) $(task_timeout_flag) $(deploy_opts)
...
halyard-auto-deploy-service: $(HOME)/.halctl
	...
	@if [ -z $(HALYARD_AUTO_DEPLOY_CLUSTER_LIST) ]; then \
		...
		svc=$(localSvc) env=$(localEnv) ver=$(halyard_ver) sleep=$(DEFAULT_SLEEP_DURATION) enableWayfarer=true taskTimeout=$(WAYFARER_TASK_TIMEOUT_IN_MINS) errorBudget=${WAYFARER_ERROR_BUDGET_COUNT} maxInflightCount=$(HALYARD_BATCH_SIZE) $(MAKE) $(MAKE_ARGS) halyard-deploy-service; \
	else \
 		echo "Going to deploy $(localSvc) $(localEnv) $(halyard_ver) on clusters $(HALYARD_AUTO_DEPLOY_CLUSTER_LIST)"; \
		svc=$(localSvc) env=$(localEnv) ver=$(halyard_ver) cluster=$(HALYARD_AUTO_DEPLOY_CLUSTER_LIST) enableWayfarer=true taskTimeout=$(WAYFARER_TASK_TIMEOUT_IN_MINS) errorBudget=${WAYFARER_ERROR_BUDGET_COUNT} maxInflightCount=$(HALYARD_BATCH_SIZE) $(MAKE) $(MAKE_ARGS) halyard-targeted-deploy; \
	fi;
...
ifneq ($(CI),true)
	...
	$(eval localTaskTimout := $(shell cat $(HALYARD_DEPLOYED_VERSIONS_DIR)/$(HALYARD_ENV_TO_DEPLOY) | $(YQ) eval '.data.taskTimeoutInMins' -))
	...
	@if [[ -z "$(localVer)" ]]; then \
		...
		svc=$(localSvc) env=$(localEnv) ver=$(localVer) sleep=$(DEFAULT_SLEEP_DURATION) enableWayfarer=$(localWay) rolloutStrategy=$(localRs) percent=$(localPer) errorBudget=$(localErrCnt) maxInflightCount=$(localmaxInflightCnt) taskTimeout=$(localTaskTimout) $(MAKE) $(MAKE_ARGS) halyard-deploy-service; \
	else \
 		echo "Going to deploy $(localSvc) $(localEnv) $(localVer) on clusters $(HALYARD_ENV_TO_DEPLOY)"; \
		svc=$(localSvc) env=$(localEnv) ver=$(localVer) cluster=$(HALYARD_CLUSTER_TO_DEPLOY) enableWayfarer=$(localWay) rolloutStrategy=$(localRs) percent=$(localPer) errorBudget=$(localErrCnt) maxInflightCount=$(localmaxInflightCnt) taskTimeout=$(localTaskTimout) $(MAKE) $(MAKE_ARGS) halyard-targeted-deploy; \
	fi;
