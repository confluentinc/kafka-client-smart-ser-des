...
# The dev ECR repos are in the same account as the prod repos.
DEVPROD_DEV_ECR_PREFIX := docker/dev
DEVPROD_DEV_ECR_REPO := $(DEVPROD_PROD_ECR)/$(DEVPROD_DEV_ECR_PREFIX)
...
ifeq ($(CI),true)
TEST_REPORT_FILE_PREFIX ?= $(BUILD_DIR)/TEST-$(SEMAPHORE_PIPELINE_ID)
else
TEST_REPORT_FILE_PREFIX ?= $(BUILD_DIR)/TEST
endif
TEST_REPORT_FILE_SUFFIX ?= result.xml
# file pattern to get all the JUnit reports (for testbreak reporting)
# this should probably be:
# TEST_REPORT_FILE_PATTERN ?= "$(TEST_REPORT_FILE_PREFIX)*/**/*$(TEST_REPORT_FILE_SUFFIX)"
# however I think there are some java/maven cases that use BUILD_DIR directly for JUnit reports.
TEST_REPORT_FILE_PATTERN ?= "$(BUILD_DIR)/**/*TEST*xml"
# file pattern to get all test reports (for semaphore reporting)
TEST_RESULT_FILE ?= $(TEST_REPORT_FILE_PREFIX)*$(TEST_REPORT_FILE_SUFFIX)
